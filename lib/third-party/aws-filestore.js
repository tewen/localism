...
function putFile({ s3Credentials, bucket, key, fileStream, contentType = 'text/plain' }) { // eslint-disable-line no-unused-vars
...
function getFile({ s3Credentials, bucket, key, writeStream }) { // eslint-disable-line no-unused-vars
  validateS3Credentials(s3Credentials , bucket);
  const myS3Credentials = _.defaults(s3Credentials, DEFAULT_S3_CREDENTIALS);
  return new Promise((resolve, reject) => {
