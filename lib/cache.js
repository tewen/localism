...
const { putFileOnS3, getFileFromS3 } = require('./aws');
...
async function getFileCacheFileFromS3(s3Credentials, s3Bucket, s3FilePath) {
...
async function putCacheFileOnS3(s3Credentials, s3Bucket, s3FilePath, contents) {
  const directoryPath = path.join(__dirname, 'tmp');
  const filePath = path.join(directoryPath, uuid.v4());
  await /* TODO: JSFIX could not patch the breaking change:
  Creating a directory with fs-extra no longer returns the path 
...
// NOTE - Should throw error / warning when incompatible params are provided
function simpleCache({ fn, transformFromFile, fileNameGenerator, getFromCacheFn, putInCacheFn, s3Credentials, s3Bucket, cacheDirectoryPath = 'cache', uuidFileName = false } = {}) {
  if (_.isFunction(fn)) {
    const cachedFn = async (...args) => {
      ...
      // NOTE - May want to clean this up and rename some params for the next major release
      const customRetrieveContents = _.isFunction(getFromCacheFn) && (await getFromCacheFn({ args, filePath }));
      const s3FileContents = !_.isEmpty(s3Credentials) && (await getFileCacheFileFromS3(s3Credentials, s3Bucket, filePath));
      if (customRetrieveContents) {
        return transformFileContents(customRetrieveContents, transformFromFile);
      } else if (s3FileContents) {
        ...
        } else if (!_.isEmpty(s3Credentials)) {
          await putCacheFileOnS3(s3Credentials, s3Bucket, filePath, stringifyIfNecessary(response));
        } else {
          await /* TODO: JSFIX could not patch the breaking change:
          Creating a directory with fs-extra no longer returns the path 
          Suggested fix: The returned promise no longer includes the path of the new directory */
          fs.mkdirp(cacheDirectoryPath);
